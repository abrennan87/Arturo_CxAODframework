...
class ObjectHandler : public ObjectHandlerBase {
  ...
  std::vector<TString> m_weightVariations; //!
  ...
  /// TODO: can we use std::function instead ? That's easier to read.
  std::vector<bool (*)(partType*)> m_selectFcns;
   ...
   * Variation dependent object variables are overwritten.
   */
  virtual EL::StatusCode setVariables(partType * inPart, partType * outPart, bool isKinVar, bool isWeightVar) = 0;
  ...
  EL::StatusCode fillOutputContainerCheck(bool (*checkSel)(partType *), const std::string& name);
...
public:
   ...
   */
  std::vector<TString> getAllVariations(bool includeNominal = true);
