...
testAthenaElectronAlg::testAthenaElectronAlg( const std::string& name, ISvcLocator* pSvcLocator ) : AthAlgorithm( name, pSvcLocator ),  m_electronSF("AsgElectronEfficiencyCorrectionTool/AsgElectronEfficiencyCorrectionTool",this){
...
//----------------------------
 ...
 ATH_CHECK( evtStore()->retrieve( electrons, "ElectronCollection") );
...
for ( xAOD::Electron* el : *(inContShallowCopy.first) ) {
  ...
  if(!m_electronSF->getEfficiencyScaleFactor(*el,SF)){
    ATH_MSG_WARNING( "Couldn't get electron scale factor!" );
  	continue;
  }

  // applyEfficiencyScaleFactor - decorate the object (*electron)
