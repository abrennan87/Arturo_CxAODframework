...
int main(int argc, char* argv[]) {
  ...
  float luminosity(0.001); // default is 0.001 fb-1, which means no rescaling of the MC inputs (already scaled to 1 pb-1)
  config->getif<float>("luminosity",luminosity);
  std::cout << "Luminosity for normalisation of MC plots = " << luminosity << " fb-1" << std::endl;
  ...
  //sample_names.push_back("ZHvv125");
  //sample_names.push_back("WH125");
  //sample_names.push_back("ZHll125");

  sample_names.push_back("ttbar");

  // datasets copied to eos
  TString CxAODver("CxAOD_00-07-00");
  ...
  std::string dataset_dir  = eosdir.Data();
  config->getif<std::string>("dataset_dir",dataset_dir);
  ...
  for (unsigned int isamp(0) ; isamp<sample_names.size() ; isamp++) {
    ...
    if (eos) {
      ...
      SH::scanSingleDir (sampleHandler, sample_name, list, "*data_CxAOD*") ;				// changed from above as Arturo's rootfiles don't have the outputlable part of the name 
    } else {
  ...
  // set the lepton analysis type, COM, and luminosity used to scale the MC samples
  algorithm->SetAnalysisType(analysisType);
  algorithm->SetCOMEnergy(comEnergy);
  algorithm->SetLuminosity(luminosity);
